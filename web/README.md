# OverSight Landing Website

Showcase website for OverSightAI - built with **React + Vite + TailwindCSS**.

## ✨ NEW: Website Revamp (v2.0) - January 2026

**Major Update**: Complete redesign with clean, modern aesthetics following shadcn principles!

### 🎯 What's New in v2.0
- ⭐ **Video Section** - Your Canva video prominently displayed after hero
- 🎨 **No Gradients** - Clean design with solid colors and subtle blur accents
- 💜 **Consistent Purple Accent** - `#7C3AED` throughout all interactive elements
- 🎭 **Enhanced Animations** - Smooth Framer Motion transitions
- 📱 **Fully Responsive** - Mobile-first design that works everywhere
- 💎 **6 Unique Testimonials** - Real quotes from diverse customers
- 📚 **Comprehensive Docs** - Complete style guide and documentation

### 📖 Documentation Files
- **[README_REVAMP.md](./README_REVAMP.md)** - Complete overview
- **[REVAMP_SUMMARY.md](./REVAMP_SUMMARY.md)** - What changed
- **[STYLE_GUIDE.md](./STYLE_GUIDE.md)** - Design system
- **[QUICK_START.md](./QUICK_START.md)** - Getting started
- **[COMPARISON.md](./COMPARISON.md)** - Before/after
- **[VISUAL_GUIDE.md](./VISUAL_GUIDE.md)** - Visual reference

---

## 🚀 Quick Start

\`\`\`bash
# Install dependencies
npm install

# Run development server
npm run dev
\`\`\`

Visit [http://localhost:5173](http://localhost:5173)

## 📁 Project Structure

\`\`\`
web/
├── src/
│   ├── components/
│   │   └── ui/           # Reusable UI components
│   ├── pages/
│   │   └── LandingPage.jsx  # Main landing page
│   ├── lib/
│   │   └── utils.js      # Utility functions
│   ├── App.jsx           # Root component
│   └── index.css         # Global styles + TailwindCSS
├── public/
│   └── OverSight.png     # Logo
└── vite.config.js        # Vite configuration
\`\`\`

## ✨ Features

- **Modern Stack**: React 18 + Vite + TailwindCSS v3
- **Animations**: Framer Motion for smooth transitions
- **Icons**: Tabler Icons
- **Responsive**: Mobile-first design
- **Dark Mode**: Built-in theme toggle
- **Components**: Reusable UI library

## 🎨 Landing Page Sections

1. **Hero** - AI Governance headline with CTA
2. **Companies** - Trust indicators
3. **Features** - Bento grid layout showcasing capabilities
4. **Testimonials** - Animated marquee
5. **Pricing** - 3-tier pricing model
6. **FAQ** - Collapsible accordions
7. **CTA** - Final call-to-action
8. **Footer** - Links and copyright

## 📦 Dependencies

- **react**: ^18.2.0
- **vite**: ^7.2.5
- **tailwindcss**: ^3.4.15
- **framer-motion**: For animations
- **@tabler/icons-react**: Icon library

## 🔧 Development

\`\`\`bash
npm run dev    # Start dev server
npm run build  # Build for production
npm run preview  # Preview production build
\`\`\`

## 🚢 Deployment

Deploy \`dist/\` directory to: Vercel, Netlify, GitHub Pages, etc.

---

**Note**: This is a standalone showcase website separate from the main OverSight application.
